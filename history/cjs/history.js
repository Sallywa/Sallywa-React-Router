...
//如果最后一个字符是“/” 那就去掉 如果不是就保持原状
function stripTrailingSlash(path) {
  //因为在addLeadingSlash函数里会给首位字符不是“/”的basename加上“/”，
  //但是有两种情况 第一是确实用户忘记加了 例如 basename={"h5"}
  //第二就是用户设置的basename为“”或者“/”（最后以为字符为addLeadingSlash添加的“/”）
  //所以这个时候需要把添加的多余的“/”给去掉
...
function createTransitionManager() {
  ...
  function appendListener(fn) {
    ...
    //listeners 是一个全局的参数 返回的函数是一个取消监听函数 即在全局liseners中把此lisener
    //剔除的举动（filter函数）
...
function createHashHistory(props) {
  ...
  //globalHistory是window的history对象的一个副本
  var globalHistory = window.history;
  //firefox里面用go会导致整个页面reload
  ...
  //监听 & 返回取消监听函数
  //学习一下这种写法 返回值是取消绑定/关联等操作，函数本身是绑定/关联
  function listen(listener) {
    var unlisten = transitionManager.appendListener(listener);
    checkDOMListeners(1);
    return function() {
